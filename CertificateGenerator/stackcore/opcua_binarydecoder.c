...
{ \
 ...
 \
    uStatus = a_pIstrm->Read(a_pIstrm, (OpcUa_Byte*)&oValue, &uBytesRead, OpcUa_Null, OpcUa_Null); \
...
{ \
    ...
    *a_pCount  = 0; \
    ...
    OpcUa_GotoErrorIfBad(uStatus); \
    \
    OpcUa_ReturnErrorIfTrue((iLength < -1), OpcUa_BadDecodingError); \
    \
    if (iLength == 0) \
    { \
        *a_pCount = 0; \
...
OpcUa_InitializeStatus(OpcUa_Module_Serializer, "OpcUa_BinaryDecoder_Open");
    ...
    *a_phDecodeContext = OpcUa_Null;
    ...
    OpcUa_GotoErrorIfArgumentNull(((OpcUa_BinaryDecoder*)a_pDecoder->Handle));
...
OpcUa_BeginErrorHandling;
    ...
    if(OpcUa_Null != ((OpcUa_BinaryDecoder*)a_pDecoder->Handle))
    {
        OPCUA_P_MUTEX_UNLOCK(((OpcUa_BinaryDecoder*)a_pDecoder->Handle)->Mutex);
    }
    ...
    OpcUa_InputStream* a_pIstrm)
{
    ...
    uBytesRead = nLength;
    uStatus = a_pIstrm->Read(a_pIstrm, (OpcUa_Byte*)pRawString, &uBytesRead, OpcUa_Null, OpcUa_Null);
    OpcUa_GotoErrorIfBad(uStatus);
    ...
    OpcUa_InputStream* a_pIstrm)
{
    ...
    uStatus = OpcUa_Int64_BinaryDecode((OpcUa_Int64*)a_pValue, a_pIstrm);
    OpcUa_GotoErrorIfBad(uStatus);
    ...
    OpcUa_InputStream* a_pIstrm)
{
    ...
    uStatus = a_pIstrm->Read(a_pIstrm, (OpcUa_Byte*)a_pValue->Data4, &uBytesRead, OpcUa_Null, OpcUa_Null);
    OpcUa_GotoErrorIfBad(uStatus);
    ...
    OpcUa_InputStream* a_pIstrm)
{
    ...
    /* read bytes of string */
    uBytesRead = nLength;
    uStatus = a_pIstrm->Read(a_pIstrm, a_pValue->Data, &uBytesRead, OpcUa_Null, OpcUa_Null);
    OpcUa_GotoErrorIfBad(uStatus);
    ...
    OpcUa_EncodeableType** a_ppType)
{
    ...
    OpcUa_InitializeStatus(OpcUa_Module_Serializer, "OpcUa_BinaryDecoder_AllocEncodeableBody");
    ...
    OpcUa_ExtensionObject* a_pValue)
{
    ...
    if (pType != OpcUa_Null)
    {
        ...
        /* set the stream position to the end of the encodeable object body */
        if (nLength > 0 && nLength > (OpcUa_Int32)(uBodyEnd - uBodyStart))
        {
            /* can only be a forward skip operation, never backwards; */
            uStatus = pHandle->Istrm->SetPosition((OpcUa_Stream*)pHandle->Istrm, uBodyStart+nLength);
            OpcUa_GotoErrorIfBad(uStatus);
        }
